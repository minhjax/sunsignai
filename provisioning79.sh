...
# ==============================================================================
# HỆ THỐNG PROVISIONING ON-DEMAND CHO VIDEO ADS CHUYÊN NGHIỆP (FULL STACK)
# Dự án: Sunsign AI - Video Engine (First -> Last Frame Workflow)
# ==============================================================================

# Đặt thư mục mặc định là chuẩn của Vast.ai / RunPod
COMFY_DIR=${COMFYUI_DIR:-"/netspace/ComfyUI"}
CUSTOM_NODES_DIR="$COMFY_DIR/custom_nodes"

echo "🚀 Bắt đầu cấu hình Video AI Engine..."
echo "📂 Thư mục gốc: $COMFY_DIR"

# ĐẢM BẢO COMFYUI CÓ SẴN (Nếu base image không có, tự động tải về)
if [ ! -d "$COMFY_DIR" ]; then
    echo "⚙️ Không tìm thấy ComfyUI. Đang tự động tải mã nguồn ComfyUI mới nhất..."
...
for node in "${!NODES[@]}"; do
    if [ ! -d "$node" ]; then
        ...
        # MẸO DEVOPS: Thêm env GIT_TERMINAL_PROMPT=0 để ngăn Git đòi mật khẩu làm treo máy
        env GIT_TERMINAL_PROMPT=0 git clone "${NODES[$node]}" || echo "⚠️ BỎ QUA: Không thể tải $node (Repo có thể đã bị xóa hoặc Private)."
    fi
done
